...
from forwarder.addons.tlsconfig import TlsConfig
...
from forwarder.test.tflow import tclient_conn
from forwarder.test.tflow import tserver_conn
from forwarder.test.tutils import tdnsreq
from forwarder.utils import data
...
def test_validation_no_transparent(monkeypatch):
    monkeypatch.setattr(forwarder.platform, "original_addr", None)
    ps = Proxyserver()
    with taddons.context(ps) as tctx:
...
def test_transparent_init(monkeypatch):
    init = Mock()
    monkeypatch.setattr(forwarder.platform, "original_addr", lambda: 1)
    monkeypatch.setattr(forwarder.platform, "init_transparent_mode", init)
    ps = Proxyserver()
    with taddons.context(ps) as tctx:
