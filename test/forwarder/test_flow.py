...
from forwarder.exceptions import FlowReadException
from forwarder.io import tnetstring
...
class TestSerialize:
    def test_roundtrip(self):
        ...
        w = forwarder.io.FlowWriter(sio)
        w.add(f)

        sio.seek(0)
        r = forwarder.io.FlowReader(sio)
        lst = list(r.stream())
        assert len(lst) == 1
    ...
    def test_filter(self):
        ...
        w = forwarder.io.FilteredFlowWriter(sio, flt)
...
class TestFlowMaster:
    ...
    async def test_all(self):
        ...
        with taddons.context(s, options=opts) as ctx:
            ...
            f.request = forwarder.test.tutils.treq()
            await ctx.master.addons.handle_lifecycle(layers.http.HttpRequestHook(f))
            assert len(s.flows) == 1

            f.response = forwarder.test.tutils.tresp()
