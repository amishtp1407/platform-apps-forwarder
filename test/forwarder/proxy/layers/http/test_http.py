...
from forwarder.http import HTTPFlow
from forwarder.http import Response
...
def test_connect_unauthorized(tctx):
    ...
    def require_auth(f: HTTPFlow):
        f.response = Response.make(
            status_code=407, headers={"Proxy-Authenticate": f'Basic realm="haltdos"'}
        )
    ...
    assert (
        ...
        << SendData(
            ...
            b'Proxy-Authenticate: Basic realm="haltdos"\r\n'
            b"content-length: 0\r\n\r\n",
        )
        >> DataReceived(
            tctx.client,
