...
def tstore(tdata):
    return certs.CertStore.from_store(
        tdata.path("forwarder/data/confdir"), "forwarder", 2048
    )
...
class TestCertStore:
    ...
    def test_chain_file(self, tdata, tmp_path):
        cert = Path(tdata.path("mitmproxy/data/confdir/forwarder-ca.pem")).read_bytes()
        (tmp_path / "forwarder-ca.pem").write_bytes(cert)
        ...
        (tmp_path / "forwarder-ca.pem").write_bytes(2 * cert)
        ca = certs.CertStore.from_store(tmp_path, "forwarder", 2048)
        assert ca.default_chain_file == (tmp_path / "forwarder-ca.pem")
