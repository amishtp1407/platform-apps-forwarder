...
import forwarder.flow
from forwarder import command
from forwarder import connection
from forwarder import ctx
from forwarder import dns
from forwarder import exceptions
from forwarder import flowfilter
from forwarder import hooks
from forwarder import http
from forwarder import io
from forwarder import tcp
from forwarder import udp
from forwarder.log import ALERT
from forwarder.utils import human
from forwarder.utils import signals
...
class _OrderKey:
    ...
    def generate(self, f: forwarder.flow.Flow) -> Any:  # pragma: no cover
        pass
...
class OrderRequestStart(_OrderKey):
    def generate(self, f: forwarder.flow.Flow) -> float:
        return f.timestamp_created


class OrderRequestMethod(_OrderKey):
    def generate(self, f: forwarder.flow.Flow) -> str:
...
class OrderKeySize(_OrderKey):
    def generate(self, f: forwarder.flow.Flow) -> int:
        if isinstance(f, http.HTTPFlow):
            size = 0
...
def _signal_with_flow(flow: forwarder.flow.Flow) -> None: ...


def _sig_view_remove(flow: forwarder.flow.Flow, index: int) -> None: ...


class View(collections.abc.Sequence):
    def __init__(self) -> None:
        super().__init__()
        self._store: collections.OrderedDict[str, forwarder.flow.Flow] = (
    ...
    def _bisect(self, f: forwarder.flow.Flow) -> int:
        v = self._view.bisect_right(f)
        return self._rev(v - 1) + 1

    def index(
        self, f: forwarder.flow.Flow, start: int = 0, stop: int | None = None
    ) -> int:
    ...
    def getvalue(self, flow: forwarder.flow.Flow, key: str, default: str) -> str:
        """
    ...
    def setvalue_toggle(self, flows: Sequence[forwarder.flow.Flow], key: str) -> None:
        """
    ...
    def setvalue(
        self, flows: Sequence[forwarder.flow.Flow], key: str, value: str
    ) -> None:
        """
    ...
    def duplicate(self, flows: Sequence[forwarder.flow.Flow]) -> None:
        """
    ...
    def remove(self, flows: Sequence[forwarder.flow.Flow]) -> None:
        """
    ...
    def resolve(self, flow_spec: str) -> Sequence[forwarder.flow.Flow]:
        """
    ...
    def load_file(self, path: forwarder.types.Path) -> None:
        """
    ...
    def add(self, flows: Sequence[forwarder.flow.Flow]) -> None:
        """
    ...
    def get_by_id(self, flow_id: str) -> forwarder.flow.Flow | None:
        """
    ...
    def update(self, flows: Sequence[forwarder.flow.Flow]) -> None:
        """
...
class Focus:
    ...
    def __init__(self, v: View) -> None:
        self.view = v
        self._flow: forwarder.flow.Flow | None = None
    ...
    @property
    def flow(self) -> forwarder.flow.Flow | None:
        return self._flow

    @flow.setter
    def flow(self, f: forwarder.flow.Flow | None):
...
class Settings(collections.abc.Mapping):
    ...
    def __getitem__(self, f: forwarder.flow.Flow) -> dict:
