...
class CompletionState(NamedTuple):
    completer: Completer
    parsed: Sequence[forwarder.command.ParseResult]


class CommandBuffer:
    def __init__(self, master: forwarder.master.Master, start: str = "") -> None:
        self.master = master
        self.text = start
    ...
    def render(self):
        ...
        if not parts:
            ...
            if remaining:
                if parts[-1].type != forwarder.types.Space:
                    ret.append(("text", " "))
    ...
    def cycle_completion(self, forward: bool = True) -> None:
        if not self.completion:
            ...
            if parts and parts[-1].type != forwarder.types.Space:
                ...
                return
            ct = forwarder.types.CommandTypes.get(type_to_complete, None)
            if ct:
...
class CommandEdit(urwid.WidgetWrap):
    ...
    def __init__(self, master: forwarder.master.Master, text: str) -> None:
