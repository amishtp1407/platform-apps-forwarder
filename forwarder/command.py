...
from forwarder import command_lexer
from forwarder import exceptions
from forwarder.command_lexer import unquote
...
def typename(t: type) -> str:
    ...
    to = forwarder.types.CommandTypes.get(t, None)
    if not to:
...
class Command:
    ...
    def __init__(self, manager: "CommandManager", name: str, func: Callable) -> None:
        ...
        for name, parameter in self.signature.parameters.items():
            ...
            if not forwarder.types.CommandTypes.get(parameter.annotation, None):
                raise exceptions.CommandError(
                    f"Argument {name} has an unknown type {t} in {func}."
                )
        if self.return_type and not forwarder.types.CommandTypes.get(
    ...
    def call(self, args: Sequence[str]) -> Any:
        ...
        if ret is None and self.return_type is None:
            return
        typ = forwarder.types.CommandTypes.get(self.return_type)
        assert typ
...
class CommandManager:
    ...
    def parse_partial(
        ...
        next_params: list[CommandParameter] = [
            CommandParameter("", forwarder.types.Cmd),
            CommandParameter("", forwarder.types.CmdArgs),
        ]
        ...
        for part in parts:
            ...
            if expected and expected.kind is inspect.Parameter.VAR_POSITIONAL:
                ...
                expected = CommandParameter("", forwarder.types.Unknown)

            arg_is_known_command = (
                expected.type == forwarder.types.Cmd and part in self.commands
            )
            arg_is_unknown_command = (
                expected.type == forwarder.types.Cmd and part not in self.commands
            )
            command_args_following = (
                next_params and next_params[0].type == forwarder.types.CmdArgs
            )
            ...
            to = forwarder.types.CommandTypes.get(expected.type, None)
            valid = False
            if to:
                try:
    ...
    def execute(self, cmdstr: str) -> Any:
        ...
        command_name, *args = (
            unquote(part.value) for part in parts if part.type != forwarder.types.Space
        )
...
def parsearg(manager: CommandManager, spec: str, argtype: type) -> Any:
    ...
    """
    t = forwarder.types.CommandTypes.get(argtype, None)
    if not t:
